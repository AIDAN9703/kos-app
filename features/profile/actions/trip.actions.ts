 ...
 *
 * Checkout charges the booking's configured amount (deposit or full), not an
 * arbitrary remainder — so this only runs while nothing has been paid yet.
 ...
 */
export async function startTripPayment(bookingId: string): Promise<StartTripPaymentResult> {
  ...
  try {
    return { success: true, url: await getOrCreateCheckoutUrl(bookingId) };
  } catch (error) {
